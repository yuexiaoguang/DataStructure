...
import com.tree.b2.BTreeNode.Entry;

/**
 * https://blog.csdn.net/u010282707/article/details/29909087
 * 
 * 一颗B树的简单实现。
 * <p/>
...
public class BTree<K, V> {
    ...
    private Entry<K, V> delete(BTreeNode<K, V> node, K key) {
        ...
        SearchResult<V> result = node.searchKey(key);
        /*
         * 因为这是查找成功的情况，0 <= result.getIndex() <= (node.size() - 1)， 因此(result.getIndex()
         * + 1)不会溢出。
         */
        if (result.isExist()) {
            ...
            /*
             * 因为这是查找失败的情况，0 <= result.getIndex() <= node.size()， 因此(result.getIndex() +
             * 1)会溢出。
             */
